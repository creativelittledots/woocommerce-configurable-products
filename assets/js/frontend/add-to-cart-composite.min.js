jQuery(document).ready(function($) {
	...
	var Component = Backbone.Model.extend({
		initialize: function(opts) {
			...
			if(this.is_style('number')) {
				this.on('change:static_value', this.update_value_selections);
				this.set('static_value', this.get('default_value'));
			} else {
		...
		update_value_selections: function() {
			...
			var title = this.get('title') + ': ' + this.get('static_value').toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + this.get('suffix'),
				max = Math.max(this.get('static_value'), this.get('min_value')),
				value = this.get('max_value') ? Math.min(max, this.get('max_value')) : max,
				error = value != this.get('static_value');
	...
	rivets.formatters['IN'] = function (value, array) {
		console.log(array);
