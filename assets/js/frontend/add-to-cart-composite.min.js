jQuery(document).ready(function($) {
	...
	var Option = Backbone.Model.extend({
		...
		display: function() {
    		this.set('display', this.get('title') + ( this.get('price') ? ' <strong>[+' + wc_cp_params.currency + this.get('price').formatMoney() + ']</strong>' : '' ) + ( this.get('is_recommended') ? ' <em>Recommended</em>' : '' ) );
		},
		select: function() {
	...
	var Component = Backbone.Model.extend({
		initialize: function(opts) {
			...
			if(this.get('default_id')) {
    			var option = this.get('options').get( this.get('default_id') );
                if( option instanceof Option ) {
    				option.set('selected', option.get('id')); //really bad 
    				this.update_selections(option);
    				this.set('first', option.get('id')); // for dropdowns
    			}
			}
			if(this.get('recommended_id')) {
	...
	var Product = Backbone.Model.extend({
		...
		evaluate: function() {
			...
			this.set('price', price.formatMoney());
